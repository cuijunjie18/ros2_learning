// 导入上一节定义的消息接口
#include "my_ros2_interfaces/msg/robot_status.hpp"
#include "my_ros2_interfaces/srv/move_robot.hpp"
#include "rclcpp/rclcpp.hpp"

/*
 * 测试指令：ros2 service call /move_robot my_ros2_interfaces/srv/MoveRobot "{distance: 5}"
 */
...
public:
  ...
  float move_distance(float distance) {
    status_ = my_ros2_interfaces::msg::RobotStatus::STATUS_MOVEING;
    ...
    status_ = my_ros2_interfaces::msg::RobotStatus::STATUS_STOP;
...
private:
  ...
  int status_ = my_ros2_interfaces::msg::RobotStatus::STATUS_STOP;
};
...
public:
  ExampleInterfacesRobot(std::string name) : Node(name) {
    ...
    /*创建move_robot服务*/
    move_robot_server_ = this->create_service<my_ros2_interfaces::srv::MoveRobot>(
      "move_robot", std::bind(&ExampleInterfacesRobot::handle_move_robot, this, std::placeholders::_1, std::placeholders::_2));
    /*创建发布者*/
    robot_status_publisher_ = this->create_publisher<my_ros2_interfaces::msg::RobotStatus>("robot_status", 10);
    /*创建一个周期为500ms的定时器*/
...
private:
  ...
  rclcpp::Service<my_ros2_interfaces::srv::MoveRobot>::SharedPtr move_robot_server_; /*移动机器人服务*/
  rclcpp::Publisher<my_ros2_interfaces::msg::RobotStatus>::SharedPtr robot_status_publisher_; /*发布机器人位姿发布者*/
  ...
  void timer_callback() {
    // 创建消息
    my_ros2_interfaces::msg::RobotStatus message;
   ...
   */
  void handle_move_robot(const std::shared_ptr<my_ros2_interfaces::srv::MoveRobot::Request> request,
                         std::shared_ptr<my_ros2_interfaces::srv::MoveRobot::Response> response) {
